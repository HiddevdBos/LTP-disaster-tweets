...
def train_model(train_x, train_y):
    ...
    train_x = train_x.reshape(num_batches, -1, 30938)
    ...
    for epoch in range(0, 10):
        for idx in range(num_batches):
            # forward step
            prediction = model.forward(train_x[idx])
...
def cross_validation(data, labels, k):
    ...
    for fold in tqdm(range(0, k), desc='folds'):
        ...
        acc_train = evaluate_model(model, train_x, train_y)
        acc_valid = evaluate_model(model, valid_x, valid_y)
